...
#%%
def call_lmstudio(system_prompt="", user_prompt="", llm_name="gemma-3n-e4b-it"):
...
#%%
def create_user_prompt(mn_text, en_text):
    translation_pair = [
...
with open(f"../datasets/{filename}", encoding='utf-8') as f:
    for el in f:
        line = el.strip().split(separator)
        mn_text = line[0].strip()
        en_text = line[1].strip()
        user_prompt   = create_user_prompt(mn_text=mn_text, en_text=en_text)
        response      = call_lmstudio(system_prompt=system_prompt, user_prompt=user_prompt)
        response_json = extract_content(response_text=response)
        score         = response_json['score']
        improved_text = response_json['final_answer']
        print(f"########################################")
        print(f"Mongolian text: {mn_text}")
